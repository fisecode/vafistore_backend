...
// Datatable (jquery)
$(function () {
  var dt_voucher_table = $('.datatables-product'),
    select1 = $('#filterBrand'),
  ...
  if (dt_voucher_table.length) {
    var dt_voucher = dt_voucher_table.DataTable({
      processing: true,
      serverSide: true,
      ajax: {
        url: baseUrl + 'dashboard/product/voucher-list',
        data: function (d) {
      ...
      <div class="d-flex justify-content-start align-items-center product-name">
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var edit = baseUrl + 'dashboard/product/voucher' + full['id'] + '/edit';
            return (
  ...
  $('.filter').on('change', function () {
    ...
    dt_voucher.column(10).search(statusFilter).column(5).search(categoryFilter).column(9).search(providerFilter).draw();
  });
  ...
  $('.btn-save-changes').on('click', function () {
    ...
    $.ajax({
      url: `${baseUrl}dashboard/product/voucher/save-bulk-edit`,
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    // get data
    $.get(`${baseUrl}dashboard/product/voucher-list\/${product_id}\/edit`, function (data) {
      let provider = '';
      if (data.provider == 4) {
  ...
  $(document).on('change', '.switch-input', function () {
    ...
    $.ajax({
      method: 'PUT',
      url: `${baseUrl}dashboard/product/voucher-list/${id}`,
      data: {
